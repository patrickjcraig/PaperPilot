...
The [current release proof index](release/PUBLIC-RELEASE-REFRESH-2026-09-02.md) distinguishes public, local and automated results. Its 36 successful receipts include all six native tool types on both Attention and GW150914, exact-source graph/annotation changes, source reopening and exact three-digest recovery through UI-only Undo/Redo. Reader-side controls in these runs were operated by an agent tester, not a human accessibility reviewer. These are repeat tests of previously used specimens, not new unseen-paper claims. The [historical `673726c` proof](release/PUBLIC-RELEASE-PROOF-2026-09-02.md), [hardening record at `274c739`](release/RECOVERY-ACCESSIBILITY-HARDENING-2026-09-02.md) and [two-tool August 30 record](release/WEBMCP-LIVE-PROOF.md) retain their original source identities and counts. Human screen-reader, literal 200% browser zoom, forced-colors/reduced-motion inspection and another-machine review remain unverified despite the completed hackathon submission.
...
The owner approved a [hackathon-only deferral](release/HUMAN-RELEASE-REVIEW-2026-09-02.md) of the four unfinished human checks. Primary/graph accessibility, literal 200% zoom and second-machine verification remain unverified, not passed; all four manifest flags remain false. These limitations remain public, and broader production acceptance still requires those checks. The [exact release-freeze procedure](release/HACKATHON-FREEZE-PLAN-2026-09-02.md) is prepared; the verified submission does not establish future enforcement of that procedure.
...
The participant reports manual application testing in **Microsoft Edge**, explicitly without a screen reader. That report does not establish native WebMCP execution in Edge, keyboard-only acceptance, literal 200% zoom or another-machine access; specific outcomes and client versions were not supplied. The [dated owner-report record](release/HUMAN-RELEASE-REVIEW-2026-09-02.md) preserves that distinction. Screen-reader acceptance remains unverified.
...
Run `npm run devpost:check -- --phase technical` for artifact/technical-proof readiness and `npm run devpost:check` for the full submission gate. The four deferred human accessibility/access flags remain false, so the strict full gate is not made green by the completed video and verified Devpost submission. Repository `npm test`, `npm run typecheck`, `npm run lint` and `npm run build` remain additional regression gates; the Next.js build is not a substitute for packaging `/webmcp/`.
