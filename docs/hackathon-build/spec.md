...
Item 10 enforces those ceilings in intake and the viewer; caller options cannot raise them. Page proxy acquisition runs in batches of four. Each canvas backing store is bounded to 8,000,000 pixels and 8,192 pixels per side without changing CSS/text/anchor coordinates; page geometry beyond 14,400 units at scale 1 rejects. Embedded text is bounded to 20,000 items/200,000 characters per page and 250,000 items/2,000,000 characters for the retained whole-paper index, including normalized text. Over-limit extraction remains a clearly limited visual source rather than silently truncated exact text. These are application allocation/retention limits, not a claim of process-level isolation of every PDF.js parser allocation.

The optional Attention demo fetch is explicit, version-pinned, streamed within the PDF byte limit, cancellable, and time-bounded. Its exact byte length and SHA-256 must match the recorded arXiv v7 specimen before opening. Ordinary local PDFs use their own computed identity, not that demo fingerprint. Intake failures use static safe messages; document replacement and page exit abort owned work. Initialization failure releases owned canvases, overlays, listeners, render tasks, and the PDF worker even if no viewer API was returned.
...
- Preserve older version-1 candidate-only copies during load, migration, and Save without hydration, deletion, or overwrite. If no version-3 or version-2 copy exists, show the explicit preserved-but-not-migrated notice; an explicit new save creates a separate version-3 copy. Only the separately confirmed human Clear action below removes known saved versions.
...
- Save/restore captures the document identity and session generation and rechecks them immediately before storage writes or hydration. Save also joins the canonical workspace transaction queue; a projection that may still roll back is never saved as successful work. Later edits cannot be labeled saved by an older completion. Paper replacement resets this paper's save opt-in, note list, order, and confirmation state.
- **Clear saved copy** requires a second, cancellable, document-bound confirmation that explicitly covers all known saved versions of this paper. The warning does not expire while it is being read and remains visible if a pending background save finishes. Confirmation invalidates pending autosaves, reads only the current SHA-qualified v1/v2/v3 keys, and removes legacy versions before the current version so they cannot reappear through fallback. It never enumerates storage, touches another paper or unknown future-version key, or deletes the active workspace. A read failure deletes nothing; a partial removal reports which known versions were removed and remain. Ordinary migration remains preservation-only. Pending recovery controls retain keyboard focus; successful Clear conditionally returns its initiating focus to Save.
...
Explanation Save/Discard remains human UI behavior, separate from graph revisions and scientific verification. It captures the visible draft identity and takeaway at activation and joins the same workspace queue as agent callbacks; a changed draft rejects rather than saving the wrong response. A 201st unique saved note rejects without evicting existing notes. Browser snapshot version 3 remains stable. Newly saved version-2 notes validate their closed shape, evidence relationships, and original response digest; absent historical sources remain incomplete audit references, while resolvable foreign/incompatible sources reject. Existing version-1 notes remain readable without invented claim classifications or retroactive digest guarantees. No PDF bytes, server writes, or local database writes are added.
...
- The trigger is the exact **Mark a region** or **Use whole page** control that opened the lens; pending selection work is generation-bound and cannot resurrect after cancellation. Duplicate annotation submission is blocked without removing keyboard focus from the initiating control. A committed annotation whose preview fails remains a real reversible edit, with an explicit preview limitation rather than a false creation failure.
...
- Loaded-workspace skip links open and focus Paper, Mentor, Graph, or Evidence directly, including the hidden rail tab when necessary. Intake and selection errors are associated with their controls; recovery success and failure remain distinct from canonical edit success.
