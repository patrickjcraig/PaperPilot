...
| DVP-02 | A working live URL is available to judges. | `publicArtifacts.liveUrl`, judge guide, and an incognito access check. | Complete for the anonymous static slice: GitHub Pages returned HTTP 200 over enforced HTTPS and the WebMCP-capable client loaded it on 2026-08-30 |
| DVP-03 | The judged Reader uses actual WebMCP tools registered with `document.modelContext.registerTool` to read one frozen, bounded paper source or visible same-paper source set and stage one structured mentor explanation for private human review. Registration alone is insufficient. | Released Reader integration, actual source-read and explanation-stage callback evidence, visible activity trail, supported-client verification, and the recorded demo. | Partial: the released arbitrary-PDF exact-text slice completed both callbacks with a visible four-event trail; figure/region/synthesis modes, durable service records, and the final recorded demo remain open |
...
| DVP-05 | An open-source license is detectable at the repository root. | Root `LICENSE`, an SPDX identifier in the manifest, and repository-home verification. | Complete: GitHub's anonymous repository and license APIs detected the root file as SPDX `MIT` on 2026-08-29 |
...
| DVP-08 | A public YouTube demo is under three minutes and has explanatory audio. | `publicArtifacts.demoVideoUrl` and manual duration/audio verification. | Open: the timed 2:30 shot plan exists, but no final public recording is claimed |
| DVP-09 | The supported WebMCP clients are named and actually tested. | `judgeExperience.testedClients`, `submissionAnswers.webMcpClients`, and dated test notes. | Complete for OpenAI Codex desktop 26.820.10647.0 in-app browser on 2026-08-30; additional clients must be added only after real release testing |
| DVP-10 | New work after 2026-08-25 is distinguishable from the existing application. | [`HACKATHON-CHANGELOG.md`](HACKATHON-CHANGELOG.md) and public dated commits. | Complete for the current slice: the pre-existing baseline remains separate and the public WebMCP release is bound to commit `c99a42d` |
...
| DVP-12 | Judges can use the app without hidden setup; any credentials are delivered only through Devpost's private field. | Judge guide plus incognito or judge-credential verification. | Complete for the current slice: the stable HTTPS demo is anonymous and requires no account, owner session, or credentials |
