...
| DVP-04 | A public GitHub, GitLab, or Bitbucket repository exposes the source and setup instructions. | `publicArtifacts.repositoryUrl` plus an incognito verification. | Pending: the isolated PaperPilot repository is ready for its first public push and anonymous verification |
...
| DVP-10 | New work after 2026-08-25 is distinguishable from the existing application. | [`HACKATHON-CHANGELOG.md`](HACKATHON-CHANGELOG.md) and public dated commits. | Blocked until PaperPilot has its own public Git history |
