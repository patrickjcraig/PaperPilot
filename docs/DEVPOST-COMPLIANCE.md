...
The canonical machine-readable requirements are in [`devpost-requirements.json`](../devpost-requirements.json). Run `npm run devpost:check -- --phase technical` for the public artifact and reproduced technical evidence, and `npm run devpost:check` for the complete gate. The current full gate is **69/73**, exit 1: technical **63/63** and submission **6/6** pass, while human review remains **0/4** under the owner's disclosed hackathon-only deferral. [PaperPilot is submitted and published on Devpost](https://devpost.com/software/paperpilot-kjglan), verified by the official submission receipt and project readback. Video verification and freeze preparation are complete. Submission and a technical pass do not certify accessibility; the scoped exception below is separate from test evidence. No checker was weakened to obtain green.
...
| DVP-02 | A working live URL is available to judges. | Manifest URL, judge guide, and clean anonymous check. | Current public source/fingerprint and matrix verified at `9dd6bd5`; another-machine human inspection remains pending |
...
The baseline acceptance gates below remain unchanged. On 2026-09-02 the owner explicitly approved a **hackathon-only exception for four unfinished human QA checks**, with public disclosure: primary keyboard/screen-reader flow, graph accessibility, literal 200% zoom, and another-machine access. See the [recorded decision](release/HUMAN-RELEASE-REVIEW-2026-09-02.md). This permits proceeding with the entry while those specific checks remain unverified; it is not full product acceptance or permission to report a green full checker. The technical/security checks and official deliverables are not waived. The [release-freeze plan](release/HACKATHON-FREEZE-PLAN-2026-09-02.md) records the exact release and preservation procedure separately from final submission.
