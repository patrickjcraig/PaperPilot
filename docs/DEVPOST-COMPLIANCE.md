...
The canonical machine-readable requirements are in [`devpost-requirements.json`](../devpost-requirements.json). Run `npm run devpost:check -- --phase technical` for the public artifact and reproduced technical evidence, and `npm run devpost:check` for the complete submission gate. The current full gate is **68/73**, exit 1: the four human checks remain unverified under the owner's disclosed hackathon-only deferral, and real Devpost confirmation remains pending. Video verification and freeze preparation are complete. A technical pass does not certify accessibility; the scoped exception below is separate from test evidence. No checker was weakened to obtain green.
