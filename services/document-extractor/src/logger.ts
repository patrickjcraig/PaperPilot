...
function safeString(value: unknown): string | undefined {
  return typeof value === "string" && SAFE_VALUE_PATTERN.test(value) ? value : undefined;
}
...
function redact(fields: SafeLogFields | undefined): Record<string, string | number> {
  ...
  for (const key of [
    "status", "sizeBytes", "durationMs", "pageCount", "chunkCount", "textBytes",
  ] as const) {
    const value = boundedInteger(fields[key]);
    if (value !== undefined) output[key] = value;
  }
  for (const key of ["requestId", "route", "method", "code", "verdict"] as const) {
...
export class JsonLineLogger implements StructuredLogger {
  ...
  info(event: string, fields?: SafeLogFields): void { this.#log("info", event, fields); }
  warn(event: string, fields?: SafeLogFields): void { this.#log("warn", event, fields); }
  error(event: string, fields?: SafeLogFields): void { this.#log("error", event, fields); }
  ...
  #log(level: "info" | "warn" | "error", event: string, fields?: SafeLogFields): void {
    ...
    try {
      ...
      // Telemetry must not affect extraction or expose a secondary raw error.
    }
  }
}
