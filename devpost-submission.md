...
The current public application is usable, with the annotation toolbar now above the PDF in deployed source `9dd6bd5`. The [150-second narrated/captioned demo](docs/demo/PaperPilot-WebMCP-demo.mp4) is recorded and encoded, with [captions](docs/demo/PaperPilot-WebMCP-demo.srt) and [media verification](docs/demo/recording-verification.json). The [current-release cross-PDF proof](docs/release/PUBLIC-RELEASE-REFRESH-2026-09-02.md) now passes technical readiness 63/63. The final YouTube URL, unanswered personal form fields, human accessibility/access review, release freeze and action-time submission confirmation remain open.
...
- Current technical readiness is **63/63** (checker exit 0), with machine-readable proof bound to `9dd6bd5` / `a0d5…` and 36 unique successful callback receipts. The evidence-validator tests pass **14/14**. This closes the source-mismatch gap using actual current-release runs; no checker or runtime rule was weakened. The full readiness check remains red on four human-review and five submission controls.
...
- **TODO:** watch and listen to the completed video, complete YouTube sign-in/publication, provide its public URL, review the complete text/fields as the participant, and obtain explicit action-time submission confirmation. Encoding and automated media verification are complete; human review is not. Preparation does not mark checklist item 12 complete or move the project into a submitted state.
