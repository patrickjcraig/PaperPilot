...
import {
  MAX_WEB_MCP_APPROVAL_PREPARATION_COMMAND_BYTES,
} from "@/server/integrations/webmcp/approval-contract";
import {
  prepareWebMcpApprovalChallenge,
} from "@/server/integrations/webmcp/approval-service";
import {
...
/** Mint a short-lived, one-use dossier after independent provider verification. */
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const result = await prepareWebMcpApprovalChallenge(
