...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  try {
    ...
    if (!workspaceId || workspaceId.length > 200 || !collectionId || collectionId.length > 200) {
      throw new HttpProblem(404, "collection_not_found", "Collection was not found.");
    }
    ...
    const result = await addWorkspacePaperToCollection(
